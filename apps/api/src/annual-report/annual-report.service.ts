...
export class AnnualReportService {
  ...
  private loadReerOfficialRows(start: Date, end: Date) {
    return this.prisma.$queryRaw<ReerOfficialAnnualRow[]>`
      SELECT
        fecha,
        EXTRACT(MONTH FROM fecha)::int AS month,
        periodo,
        version,
        eopreer_eur AS "eopreerEur",
        s_imp AS "sImp"
      FROM omie_reer_official_annotations
      WHERE fecha >= ${start}
        AND fecha < ${end}
        AND periodo > 0
        AND codigo_documento = '9230'
        AND estado::text = ${OmieDownloadEstado.PROCESADO}
        AND agente = ${STROM_AGENT}
      ORDER BY fecha ASC, version DESC, periodo ASC
    `;
  }
...
function aggregateReerOfficialByMonth(rows: ReerOfficialAnnualRow[]) {
  ...
  const map = new Map<number, Pick<MonthValues, "importeReerEur">>();
  for (const row of rows) {
    if (row.version !== latestVersionByDate.get(formatDateOnly(row.fecha))) {
      continue;
    }
    const bucket = map.get(row.month) ?? { importeReerEur: null };
    bucket.importeReerEur = addNullable(bucket.importeReerEur, decimalToNumber(row.eopreerEur) * officialSign(row.sImp));
    map.set(row.month, bucket);
  }
  return map;
}
